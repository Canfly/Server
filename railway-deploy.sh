...
RUN echo '#!/bin/sh' > /docker-entrypoint.d/40-railway-port.sh && \\
    echo 'sed -i "s/listen 80/listen \${PORT:-80}/g" /etc/nginx/conf.d/default.conf' >> /docker-entrypoint.d/40-railway-port.sh && \\
    echo 'exec "$@"' >> /docker-entrypoint.d/40-railway-port.sh && \\
...
# Создаем новый проект в Railway
info "Создаем новый проект в Railway..."
...
if [ $? -ne 0 ]; then
  ...
  railway link --name "$PROJECT_NAME"  
  if [ $? -ne 0 ]; then
    error "Не удалось привязать проект к локальной директории."
  fi
fi
